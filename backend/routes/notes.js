...
router.delete('/deletenote/:id', fetchuser, async (req, res) => {
    // const { title, description, tag } = req.body;
    try {
        // Find the note to be delted and delete them
        let note = await Note.findById(req.params.id) //it is the id of the user who want to update the note
        if (!note) {
            return res.status(404).send("Not Found")
        }
        ...
        if (note.user.toString() !== req.user.id) { //This line checks if the user making the request is the owner of the note. It compares the ID of the authenticated user (req.user.id) with the ID of the user who created the note ('note.user'). If they do not match, it returns a 401 error response indicating that the user is not allowed to delete the note.
            return res.status(401).send("Not Allowed")
        }
